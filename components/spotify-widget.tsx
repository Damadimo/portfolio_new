...
export function SpotifyWidget() {
  ...
  return (
    ...
    >
      <div className="relative flex items-center justify-center w-10 h-10 rounded-md bg-[#1DB954]">
      ...
      <div className="min-w-0">
        <p className="text-base text-foreground truncate">{currentTrack.name}</p>
        <p className="text-sm text-muted-foreground truncate">{currentTrack.artist}</p>
      </div>
      <svg 
        viewBox="0 0 24 24" 
        className="w-5 h-5 text-muted-foreground group-hover:text-foreground transition-colors ml-auto shrink-0"
        fill="none"
        stroke="currentColor"
        strokeWidth="2"
      >
